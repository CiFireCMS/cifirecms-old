...
class Alert {
	...
	public function show($alert_name = '', $type = '', $content = '', $timeout=TRUE, $static = FALSE) 
	{
		...
		if ($static==TRUE)
		{
			$alert = $this->alert($type, $content, $timeout=TRUE);
		}
	...
	{
		$scriptTmieout = $tmieout==TRUE?'<script>$("#alert-timeout").delay(90000).slideUp(300, function() {$(this).alert("close");$(this).remove();});</script>':'';
		$alert = '<div id="alert-timeout" class="alert alert-'.$type.' alert-styled-left alert-arrow-left alert-dismissible"><button type="button" class="close" data-dismiss="alert"><span>×</span></button>'.$content.'</div>'.$scriptTmieout;
		return $alert;
	}
} // End class