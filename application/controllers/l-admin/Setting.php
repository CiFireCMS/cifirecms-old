...
class Setting extends Backend_Controller {
	...
	public function index()
	{
		if ($this->read_access)
		{
			...
			{
				if ($this->input->post('act')=='metasocial' && $this->modify_access && $this->write_access)
				{
				...
				elseif($this->input->post('act')=='backupSystem' && $this->modify_access && $this->write_access)
				{
					return $this->backup();
				}
				else
	...
	public function createSitemap()
	{
		if ($this->read_access && $this->write_access)
		{
			if ($_SERVER['REQUEST_METHOD']=='POST' && $this->input->post('pk')=='sitemap')
			{
				...
				$this->alert->set($this->mod, 'info', lang_line('_sitemap_was_created'));
	...
	public function list()
	{
		...
		if ($this->read_access == TRUE)
		{
			...
			{
				$this->render_view('view_listable', $this->vars);
			}
		}
		else
		{
				...
				if ($_SERVER['REQUEST_METHOD']=='POST')
				{
					...
					if ( $this->form_validation->run() ) 
					{
						...
						if ($this->setting_model->update_setting($id,$data_insert))
						{
							...
							$this->alert->set($this->mod, 'info', lang_line('form_message_update_success'));
	...
	{
		if ($this->read_access && $this->input->is_ajax_request())
		{
	...
	{
		if (group_active()=='root' && $_SERVER['REQUEST_METHOD']=='POST')
		{
			...
			{
				$session['filez'] = "$bakName.zip";
				$this->session->set_flashdata($session);
				redirect(admin_url($this->mod."/downloadz"),'location', 302);
			}
		}
		else
		{
	...
	{
		if (group_active()=='root' && !empty($_SESSION['filez']))
		{
			$filez = $this->CI->session->flashdata('filez');
