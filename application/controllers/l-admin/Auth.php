...
class Auth extends MY_Controller {
	...
	public function index()
	{
		...
		else
		{
			...
			else
			{
				...
				$this->load->view('admin/auth_header', $this->vars);
				$this->load->view('admin/auth_login', $this->vars);
				$this->load->view('admin/auth_footer', $this->vars);
			}
		}
	}
	...
	private function _submit_login($name = NULL, $value = NULL)
	{
		    ...
		    )
		{
			...
			if ( $this->form_validation->run() ) 
			{
				...
				{
					$this->alert->set('login', 'warning', lang_line('message_login_error'));
			...
			{
				$this->alert->set('login', 'warning', validation_errors());
				redirect(uri_string(),'location',302);
			}
		}
	...
	public function forgot()
	{
		...
		{		
			if ( $_SERVER['REQUEST_METHOD'] == 'POST' )
			{
				...
				if ( $this->form_validation->run() )
				{
					...
					if ( $query->num_rows() == 1 )
					{
						...
						$this->alert->set('forgot', 'info', lang_line('message_forgot_success'));
						// redirect(uri_string());
					} 
					else
					{
						$this->alert->set('forgot', 'warning', lang_line('message_forgot_error'));
					}
			...
			{
				$this->load->view('admin/auth_header', $this->vars);
				$this->load->view('admin/auth_forgot', $this->vars);
				$this->load->view('admin/auth_footer', $this->vars);
			}
		}
	}
