...
class Theme extends Backend_Controller {
	...
	public function index()
	{
		if ( $this->read_access ) 
		{
			...
			elseif ( $this->_act == 'active' && $this->modify_access )
			{
				...
				$this->alert->set($this->mod, 'info', lang_line('message_active_success'));
				redirect(uri_string());
			}
	...
	private function _create_blank_theme()
	{
		...
		if ( $this->form_validation->run() )
		{
			...
			$this->alert->set($this->mod, 'info', lang_line('message_add_success'));
		}
		else
		{
		...
		if ( $this->modify_access )
		{
			...
			if ( !empty($this->vars['res_theme']) ) 
			{
				...
				if ($this->_act == 'edit') 
				{
					...
					// alert
					$this->alert->set($this->mod,'info', "File <code>".$file.".php</code> ".lang_line('form_message_update_success'));
	...
	public function backup()
	{
		if ( $this->input->is_ajax_request() && $this->read_access && $this->write_access ) 
		{
			...
			$dir_temp_theme = "themez".date('Ymdhis').md5($theme_title);
			...
			@r_copy($this->_path['temp'].$zip_name, $this->_path['uploads'].$zip_name);
			@delete_folder($this->_path['temp'].$zip_name);
			@delete_folder($path_dir_temp);

			$this->load->helper('download');
			force_download($this->_path['uploads'].$zip_name, NULL);
