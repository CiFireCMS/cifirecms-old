...
class Component extends Backend_Controller {
	...
	private function _delete()
	{
		...
		else
		{
			$response['success']          = false;
			$response['alert']['type']    = 'danger';
			$response['alert']['content'] = 'ERROR 403';
			$this->json_output($response);
		}
	}
	...
	public function installation()
	{
		if ( $this->read_access && $this->write_access && $this->modify_access && $this->delete_access )
		{
			...
			else
			{
				...
				$this->render_view('view_installation', $this->vars);
			}
		}
		else
		{
	...
	private function _install()
	{
		if ( $this->read_access && $this->write_access && $this->modify_access && $this->delete_access )
		{
			...
			if ( $this->upload->do_upload('file') )
			{
					...
					)
				{
						...
						)
					{
						...
						if (!empty($_config['file_sql']))
						{
							...
							{
								@delete_folder($destinationdir_unzip); // delete folder temp.
								$this->alert->set($this->mod, 'danger', '<i class="cificon licon-alert-triangle mr-2"></i>SQL Error! Or access denied.! Check the sql file and import it again');
								redirect(admin_url($this->mod));
							}
						}
						else
						{
							@delete_folder($destinationdir_unzip); // delete folder temp.
							$this->alert->set($this->mod, 'warning', '<p class="tx-medium"><i class="cificon licon-alert-triangle mr-2"></i>WARNING.!</p>Some mod sources is corrupt.! Check your system or contact the administrator.');
							redirect(admin_url($this->mod));
						}
					}
					...
					{
						@delete_folder($destinationdir_unzip); // delete folder temp.
						$this->alert->set($this->mod, 'danger', lang_line('err_install_package'));
						redirect(uri_string());
					}
				}
				...
				{
					@delete_folder($destinationdir_unzip); // delete folder temp.
					$this->alert->set($this->mod, 'danger', lang_line('err_config_notfound'));
					redirect(uri_string());
				}
			}
	...
	public function backup($id = '')
	{
		if ($this->read_access && $this->write_access && $this->modify_access && $this->delete_access)
		{
			...
			{
				$this->alert->set($this->mod, 'danger', lang_line('err_component_notfound'));
				redirect(admin_url($this->mod));
			}
		}
		else
	...
	private function _runBackup($component)
	{
		...
		$dir_name = "modz".date('Ymdhis').md5($c_class);
		...
		if ( force_download(CONTENTPATH.'uploads/'.$zip_name, NULL) ) 
		{
