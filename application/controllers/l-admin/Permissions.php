...
class Permissions extends Backend_Controller {
	...
	private function list_group()
	{
		...
		if ($this->read_access)
		{
			...
			{
				if ($this->input->post('act') == 'delete' && $this->delete_access)
				{
			...
			{
				$this->render_view('list_group', $this->vars);
			}
		}
		else
		{
	...
	public function detail_group($pk='')
	{
		...
		if ($this->read_access) 
		{
			...
			if ($cek==1)
			{
				...
				$this->render_view('view_group', $this->vars);
			}
	...
	public function add_group()
	{
		...
		if ($this->write_access)
		{
			...
			{
				$this->render_view('add_group', $this->vars);
			}
		}
		else
		{
		...
		if ($this->modify_access && $pk!='root') 
		{
			if ($this->permissions_model->cek_pk($pk)==1)
			{
				...
				else
				{
					...
					$this->render_view('edit_group', $this->vars);
				}
			}
			else
			{
	...
	public function role($pk='')
	{
		...
		if ($this->read_access && $this->delete_access)
		{
			$pk = xss_filter($pk,'xss');
			$cek = $this->permissions_model->cek_pk($pk);
			...
			if ( $cek == 1 && $pk!='root')
			{
				...
				else
				{
					$this->vars['res_group'] = $res_group;
					$this->vars['res_roles'] = $this->permissions_model->get_roles_by_group($res_group['group']);
					$this->render_view('role', $this->vars);
				}
			}
			else
			{
	...
	public function list_role()
	{
		...
		if ($this->read_access)
		{
			...
			{
				$this->render_view('list_role', $this->vars);
			}
		}
		else
		{
	...
	public function add_group_role($groupPk='')
	{
		...
		if ($this->write_access)
		{
			...
			if ( $cekGroup == 1)
			{
				...
				{
					$this->vars['res_group'] = $res_group;
					$this->render_view('add_group_role', $this->vars);
				}
			}
			else
			{
		...
		if ($this->modify_access)
		{
			...
			if ( $queryRole->num_rows() == 1 )
			{
				...
				else
				{
					...
					$this->render_view('edit_group_role', $this->vars);
				}
			}
			else
			{
	...
	public function add_list_role()
	{
		...
		if ($this->write_access)
		{
			...
			{
				$this->render_view('add_list_role', $this->vars);
			}
		}
		else
		{
		...
		if ($this->modify_access)
		{
			...
			if ( !is_null($getRole) )
			{
				...
				else
				{
					...
					$this->render_view('edit_list_role', $this->vars);
				}
			}
			else
			{
